...
class RepVGGBlock(nn.Module):
    ...
    def get_equivalent_kernel_bias(self):
        """获取等效的融合kernel和bias - 严格对齐PyTorch"""
    ...
    def switch_to_deploy(self):
        ...
        self.rbr_reparam.weight = kernel
...
class RealVGGBlock(nn.Module):
    """RealVGGBlock - 简单的VGG风格block，严格对齐PyTorch"""
...
class ScaleLayer(nn.Module):
    """ScaleLayer - 可学习的缩放层，严格对齐PyTorch"""

    def __init__(self, num_features, use_bias=True, scale_init=1.0):
        super(ScaleLayer, self).__init__()
        self.weight = Parameter(jt.full((num_features,), scale_init))
        self.num_features = num_features
        if use_bias:
            self.bias = Parameter(jt.zeros((num_features,)))
        else:
            self.bias = None
...
class LinearAddBlock(nn.Module):
    """LinearAddBlock - A CSLA block is a LinearAddBlock with is_csla=True，严格对齐PyTorch"""
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1,
        ...
        if is_csla:  # Make them constant
            self.scale_1x1.weight.stop_grad()
            self.scale_conv.weight.stop_grad()
        if use_se:
            raise NotImplementedError("se block not supported yet")
        else:
            self.se = nn.Identity()
